...
The dispersion parameter \phi in the negative binomial model controls how much variability exceeds the Poisson assumption. While \phi is weakly identified in practice, its impact can be understood through the concept of effective sample size. Smaller values of \phi inflate variance, making the data behave as if fewer samples were collected. For example, with \phi = 50, the variability is roughly equivalent to halving the number of observed fish compared to a Poisson model (check). This provides an intuitive way to interpret overdispersion: it reduces the information content of the data, as though sampling effort were lower.

In practice, overdispersion reduces the effective sample size, so the data behave as if fewer fish were measured.
