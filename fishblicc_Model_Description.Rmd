...
3. Mortality is fixed within length intervals, but may vary between intervals. Intervals can be any width. Because mortality need only be fixed inside intervals, different selectivities among any number of fishing gears and other changes in mortality such as size related natural mortality can be accounted for relatively easily. A general parametric selectivity function is suggested (double-sided normal) that has been used to account for a wide range of selectivity that might be encountered.  

One of the motivations for developing this model was to make it simple enough to fit the model to length frequency data using a Bayesian approach that accounts for uncertainties.  

# The Population Model  

This section deals with the effect of length-based mortality (e.g. selectivity, length dependent natural mortality) on mortality on the length composition. The aim is to generalized the mortality function so that the effect of variable mortality on the length composition is accounted for. To generalize accounting for changing mortality with length due to selectivity, it is necessary to account for two processes: 1) change in mortality due to selectivity or other effects 2) deceleration of growth as fish get larger.  
...
For ease of reference, the subscript $k$ is dropped for the mortality parameter, but note that time is effectively measured in units of the growth rate parameter $k$.  
...
The function is calculated at each Gauss-Laguerre node for $x$, and the weighted sum provides an estimate of the integral. Tests of the Gauss-Laguerre integral found 110 nodes provided stable results across the whole range of parameters that would be apply in this model, but fewer nodes may be adequate dependent on the desired accuracy. Fewer nodes reduces the calculation cost proportionately.  
